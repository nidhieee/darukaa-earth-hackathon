...
from api.models import User, Project, Site, AnalyticsSnapshot
...
def seed_db():
    ...
    proj1 = Project(owner_id=user.id, name="Western Ghats Reforestation", description="Biodiversity hotspot restoration")
    proj2 = Project(owner_id=user.id, name="Mangrove Conservation", description="Coastal protection and carbon sink")
    ...
    sites_data = [
        ...
        (proj2.id, "Coastal Site", "POLYGON((80.2 13.0, 80.21 13.0, 80.21 13.01, 80.2 13.01, 80.2 13.0))")
    ]
    
    sites = []
    for pid, name, wkt in sites_data:
        area_m2 = db.execute(text("SELECT ST_Area(ST_GeomFromText(:wkt, 4326)::geography)"), {"wkt": wkt}).scalar()
